...
const config = {
  ...
  remoteApiDisplayName: env.REMOTEAPI_DISPLAYNAME,
  remoteApiURL: env.REMOTEAPI_URL,
  remoteApiRejectInvalidCert: env.REMOTEAPI_REJECTINVALIDCERT,
  directConnectApiIPv4: env.DIRECTCONNECTAPI_IPV4,
  directConnectApiPort: env.DIRECTCONNECTAPI_PORT,
  directConnectApiHttps: env.DIRECTCONNECTAPI_HTTPS,
  directConnectApiHttpsKeyPath: env.DIRECTCONNECTAPI_HTTPSKEYPATH,
  directConnectApiHttpsCertPath: env.DIRECTCONNECTAPI_HTTPSCERTPATH,
  jwt: env.JWT,
  jwtAudience: env.JWT_AUDIENCE,
  jwtIssuer: env.JWT_ISSUER,
  jwtClientID: env.JWT_CLIENTID,
  jwtClientSecret: env.JWT_CLIENTSECRET,
};
  ...
  if (!isNaN(config.directConnectApiPort) && config.directConnectApiPort >= 1024 && config.directConnectApiPort <= 63335){
    if (config.directConnectApiHttps === true){
     ...
     try {
      ...
      try {
        fs.accessSync (`${config.directConnectApiHttpsKeyPath}`, fs.constants.R_OK);
        const key = fs.readFileSync(`${config.directConnectApiHttpsKeyPath}`);
        options['key'] = key;
      } catch (err) {
